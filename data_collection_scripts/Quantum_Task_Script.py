...
if subInfo['method'] == 'Behavioral':
    ...
    Quantum_Task_Parameters = {'respKeys': resp_keys, 'n_blocks': 5, 'n_trials': 40, 'n_prac_trials': 40, 'switchRange': [10,20], 
                                'easy':[10, 5, 0, 0, 0, 0], 
                                'med':[10,10,10, 9, 9, 9], 
...
def generate_state(Quantum_Task_Parameters, subInfo):
    hidden_state = []
    ...
    switch_counter=0
    while switch_counter != 13: # repeat state generation until switch counter is 13 ... aka all subjects should switch 13 times over the 5 blocks
        switch_counter=0 # set at 0 again in case we need to repeat code below this
        hidden_state = [] # reset at empty again just in case we need to repeat code below this
        ...
        print("repeating state generation")
    ...
    with open(os.path.join(output_dir,("Data_"+subInfo['method']),("sub-" + str(int(subInfo['Subject_ID'])) + '_state_array.npy')), 'wb') as f:
        np.save(f, hidden_state) # what state they were in
...
def prep_Quantum_block_trials(Quantum_Task_Parameters, subInfo, state, i_block):
    ...
    if state == []:
        Quantum_Output['state'] = generate_state(Quantum_Task_Parameters, subInfo)
...
if subInfo['method']!="MRI":
    ...
    if subInfo['current_task']=="Quantum":
        ...
        if do_task==1:
            # Prep block trials
            state = generate_state(Quantum_Task_Parameters, subInfo)
    ...
    ###################################################### Run trials #########################################################
    if subInfo['Block'] == 1:
        # if starting from block 1, we need to generate state
        state = generate_state(Quantum_Task_Parameters, subInfo)
    else:
        # if starting from any block other than 1, code probably crashed and we need to load the state
        with open(os.path.join(output_dir,("Data_"+subInfo['method']),("sub-" + str(int(subInfo['Subject_ID'])) + '_state_array.npy')), 'rb') as f:
            state=np.load(f)
